...
GitHub strips inline audio players from normal repository READMEs, so the samples are linked directly as WAV files. These three clips are checked-in renders from the current engine, regenerated from the latest code rather than copied from an older revision.
...
| Mounted-case sequential read stream | One larger initial seek lands on data, then the drive stays in a smoother sequential read pattern with small periodic track-boundary corrections. This sample is rendered with the `mounted_in_case` acoustic profile. | Most of what you hear is steady spindle and enclosure motion with lighter sampled-servo correction ticks at boundaries. It is meant to sound more like a damped case-mounted drive than a bright exposed mechanism. | [sequential-read-stream.wav](samples/sequential-read-stream.wav) |
| Bare-drive random seek + journal + flush burst | A busy burst of short and medium seeks mixes ordinary data traffic with journal-like updates and heavier flush-style events. This sample is rendered with the `bare_drive_lab` acoustic profile. | The bare-drive profile exposes more direct actuator and cover resonance, so the servo-wedge ticks and short seeks come through more sharply. Flush-heavy moments sound denser because the head is being driven through more abrupt repositioning and settle behavior. | [random-seek-journal-flush.wav](samples/random-seek-journal-flush.wav) |
