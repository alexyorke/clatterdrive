...
def _verify_trace(path: Path) -> None:
...
def _umount_macos(mount_point: Path) -> None:
    result = subprocess.run(["umount", str(mount_point)], capture_output=True, text=True, check=False)
    if result.returncode == 0:
        return
    subprocess.run(["diskutil", "unmount", "force", str(mount_point)], capture_output=True, text=True, check=False)


def run_macos_mount_e2e(backend_exe: Path | None = None, *, space_paths: bool = False) -> dict[str, object]:
    if platform.system() != "Darwin":
        raise RuntimeError("mount_webdav E2E requires macOS")
    if shutil.which("mount_webdav") is None:
        raise RuntimeError("mount_webdav was not found on PATH")
    prefix = "clatterdrive mac e2e spaces " if space_paths else "clatterdrive-mac-e2e-"
    with tempfile.TemporaryDirectory(prefix=prefix) as temp:
        root = Path(temp)
        backing_dir, tee_path, trace_path = _workspace_paths(root, space_paths=space_paths)
        mount_point = root / "mounted webdav"
        mount_point.mkdir()
        port = _free_port()
        process = _launch_backend(backend_exe, port, backing_dir, tee_path, trace_path)
        mounted = False
        try:
            ...
            result = subprocess.run(
                ["mount_webdav", f"{base_url}/", str(mount_point)],
                capture_output=True,
                text=True,
                check=False,
            )
            if result.returncode != 0:
                raise RuntimeError(f"mount_webdav failed: {result.stdout}\n{result.stderr}")
            mounted = True
            mounted_file = mount_point / "macos-mounted-e2e.bin"
            payload = b"hello from macos mount_webdav e2e"
            mounted_file.write_bytes(payload)
            time.sleep(0.8)
            if mounted_file.read_bytes() != payload:
                raise AssertionError("mount_webdav read did not match written bytes")
            downloaded = _request("GET", f"{base_url}/macos-mounted-e2e.bin")
            if downloaded != payload:
                raise AssertionError("WebDAV GET did not match mount_webdav write")
            mounted_file.unlink()
        finally:
            if mounted:
                _umount_macos(mount_point)
            _shutdown_backend(process, f"http://127.0.0.1:{port}")
        _verify_wav(tee_path)
        _verify_trace(trace_path)
        return {
            "ok": True,
            "macos_mount": str(mount_point),
            "port": port,
            "ready": ready,
            "space_paths": space_paths,
            "tee_bytes": tee_path.stat().st_size,
...
def main() -> None:
    ...
    parser.add_argument("--mapped-drive", action="store_true", help="Use Windows WebClient and net use for mapped-drive E2E.")
    parser.add_argument("--macos-mount", action="store_true", help="Use macOS mount_webdav for mounted-volume E2E.")
    parser.add_argument("--space-paths", action="store_true", help="Use backing, tee, and trace paths containing spaces.")
    args = parser.parse_args()
    if args.mapped_drive and args.macos_mount:
        raise SystemExit("--mapped-drive and --macos-mount are mutually exclusive")
    if args.mapped_drive:
        result = run_mapped_drive_e2e(args.backend_exe, space_paths=args.space_paths)
    elif args.macos_mount:
        result = run_macos_mount_e2e(args.backend_exe, space_paths=args.space_paths)
    else:
        result = run_backend_e2e(args.backend_exe, space_paths=args.space_paths)
